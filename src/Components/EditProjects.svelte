  ...
  <div id="carousel">
    ...
    <Carousel swiping={false}>
      {#each projectsArray as project, index}
        <ProjectForm project={project} index={index} addProject={() => {projectsArray.push(emptyProject); projectsArray = projectsArray; console.log(projectsArray, $projects)}} last={index === $projects.length - 1}/>
      {/each}
    </Carousel>
    {/if}
  </div>
